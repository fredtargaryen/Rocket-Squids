    ...
    public void aiStep() {
        ...
        if (this.level().isClientSide()) {
            ...
            if (this.getEntityData().get(BLASTING)) {
                ...
                this.tentacleAngle = this.isInWater() ? (float) ((Math.PI / 6.0) + (Mth.sin((float) Math.toRadians(4 * (this.tickCount % 360))) * Math.PI / 6)) : 0;
            }
            if (this.getEntityData().get(BLASTING)) {
                if (this.isInWater()) {
        ...
        if (!this.level().isClientSide()) {
            ...
            if (interactStack == ItemStack.EMPTY) {
                ...
                if (interactItem == RSItems.SQUELEPORTER_INACTIVE.get()) {
                    ...
                    // Set squid data
                    CompoundTag squidData = new CompoundTag();
                    this.save(squidData);
                    this.addAdditionalSaveData(squidData);
                    newStack.set(SQUELEPORTER, new SqueleporterData(squidData));
        ...
        if (!this.level().isClientSide()) {
            ...
            switch (this.uuid.compareTo(partnerUUID)) {
                ...
                case 1:
                    ...
                    this.breedCooldown = CommonConfig.BREED_COOLDOWN; // the breed cooldown is set in the config file
